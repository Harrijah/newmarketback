  ...
  function filtermylist(selector) {
    const templist =
      ...
      Array.from(allproductslist)
        ...
        }); // posiiton : attribut rajouté arbitrairement React n'est pas content
    ...
    if (filteredlist == []) {
      const templist =
        ...
        Array.from(filteredlist).map((product, index) => (
          <div
            key={product.id}
            className={index != sliderindex ? "slidebox" : "showme"}
          >
            <img
              src={
                !isEmpty(allproductslist)
      ...
      const templist =
        ...
        Array.from(filteredlist)
          .filter((product) => (product.position > -1 && product.position < 3 ))
          .map((product) => (
            <div key={product.id} className={`showme image0${product.position}`} style={{
                  left: product.position == 0 ? gauche : ((product.position == 1) ? centre : (product.position == 2 && droite)
                  )
                }}>
